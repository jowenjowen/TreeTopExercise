...
public class OrganizationListTest {
    ...
    public void getSqlStringTest() throws Exception {
        ...
        // test order by and direction
        assertEquals("select * from organizations where lower(state)=lower('FL') order by upper(city);", sut.getSqlString(0, "", "", "FL", "", "", "city", ""));
        assertEquals("select * from organizations where lower(state)=lower('FL') order by upper(city) DESC;", sut.getSqlString(0, "", "", "FL", "", "", "city", "DSC"));
    }
}